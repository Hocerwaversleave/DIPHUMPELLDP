...
iocon(void) {
	...
	DDRE = _BV(SIGNALPRIMARYTRANSMITTER);
	DDRF = _BV(MOTOR_L_FWD) | _BV(MOTOR_L_BWD) | _BV(MOTOR_R_FWD);
	...
	TC4H = 0x03;
	OCR4C = 0xFF;
	TC4H = (((uint16_t)SPEEDLEFTINIT) >> 8);
	OCR4B = (uint8_t)(SPEEDLEFTINIT);
	OCR4D = (uint8_t)(SPEEDRIGHTINIT);
	TCCR4E &= ~_BV(TLOCK4);
